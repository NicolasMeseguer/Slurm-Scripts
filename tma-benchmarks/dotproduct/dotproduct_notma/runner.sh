...
#SBATCH --job-name=tma_dotproduct_notma
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
for CU in 1 2 4 5 8 16 32 64 120; do
  ...
  for TILESIZE in 512 1024 2048 4096; do
  
    ./dotproduct_notma -timing -report-all -magic-memory-copy -metric-file-name="${1}-slurm-${CU}cu-${TILESIZE}" -length=$BASE_LENGTH -tilesize=$TILESIZE -globalsize=$GLOBALSIZE -localsize=$LOCALSIZE -gpu-model="$1"
