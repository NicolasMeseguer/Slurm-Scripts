...
#SBATCH --job-name=tma_dotproduct_queue_configurator_driver
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
for CU in 1 2 4 5 8 16 32 64 120; do
  ...
  ./dotproduct_queue_configurator_driver -timing -report-all -magic-memory-copy -metric-file-name="${1}-slurm-${CU}cu" -length=$BASE_LENGTH -globalsize=$GLOBALSIZE -localsize=$LOCALSIZE -gpu-model="$1"
done
