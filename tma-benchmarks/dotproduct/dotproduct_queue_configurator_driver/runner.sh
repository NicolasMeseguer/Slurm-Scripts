...
for CU in 1 2 4 5 8 16 32 64 120; do
  ...
  ./dotproduct_queue_configurator_driver -timing -report-all -magic-memory-copy -metric-file-name="slurm-${CU}cu" -length=$BASE_LENGTH -globalsize=$GLOBALSIZE -localsize=$LOCALSIZE -gpu-model="$1"
done
