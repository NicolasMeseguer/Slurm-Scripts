...
for CU in 1 2 4 5 8 16 32 64 120; do
  ...
  for QUEUETILE in 1 2 4 8; do
    for TILESIZE in 512 1024 2048 4096 8192; do
      ...
      ./sumvectors_queue -timing -report-all -magic-memory-copy -metric-file-name="slurm-${CU}cu-${QUEUETILE}qt-${TILESIZE}" -length=$LENGTH -tilesize=$TILESIZE -queue_tiles=$QUEUETILE -globalsize=$GLOBALSIZE -localsize=$LOCALSIZE -gpu-model="$1"
