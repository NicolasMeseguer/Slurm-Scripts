...
for CU in 1 2 4 5 8 16 32 64 120; do
  ...
  for TILESIZE in 64 128 256 512 1024 2048; do
    ...
    ./matrixvector_notma -timing -report-all -magic-memory-copy -metric-file-name="${1}-slurm-${CU}cu-${TILESIZE}" -dim_m=$DIM_M -dim_k=$DIM_K -TileSizeK=$TILESIZE -globalsize=$GLOBALSIZE -localsize=$LOCALSIZE -gpu-model="$1"
