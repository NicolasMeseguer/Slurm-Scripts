...
for CU in 1 2 4 5 8 16 32 64 120; do
  ...
  for QUEUETILE in 1 2 4; do
    for TILESIZE in 64 128 256 512; do
      ...
      ./matrixvector_queue -timing -report-all -magic-memory-copy -metric-file-name="slurm-${CU}cu-${QUEUETILE}qt-${TILESIZE}" -dim_m=$DIM_M -dim_k=$DIM_K -TileSizeK=$TILESIZE -queue_tiles=$QUEUETILE -globalsize=$GLOBALSIZE -localsize=$LOCALSIZE -gpu-model="$1"
